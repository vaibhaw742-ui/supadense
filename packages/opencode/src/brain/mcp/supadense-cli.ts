#!/usr/bin/env bun
// supadense CLI — run with: bun run packages/opencode/src/brain/mcp/supadense-cli.ts <command>
//
// Commands:
//   login          — authenticate and save API key to ~/.supadense/config.json
//   init [--name]  — register current dir as a local project + write .mcp.json
//   status         — show registered projects and whether cwd is registered
