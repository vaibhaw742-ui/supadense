...
export default function WorkspaceGraph() {
  ...
  // Rebuild graph whenever data or container dimensions are ready
  createEffect(() => {
    ...
    // Wait for container to have real dimensions (it may be 0 on first paint)
    const tryDraw = () => {
      if (!container) return
  ...
  function drawGraph(nodes: ProjectNode[], w: number, h: number) {
    ...
    const radius = (n: ProjectNode) => Math.max(32, Math.min(72, 32 + n.doc_count * 7))
    ...
    const sim = d3.forceSimulation<ProjectNode>(nodes)
      .force("charge",  d3.forceManyBody<ProjectNode>().strength(-400))
      .force("center",  d3.forceCenter(w / 2, h / 2))
      .force("collide", d3.forceCollide<ProjectNode>((n) => radius(n) + 24).strength(0.8))
      .alphaDecay(0.025)
    simRef = sim
    ...
    node.append("text")
      ...
      .attr("dy",                 "-8")
      .attr("fill",               (d) => d.color)
      .attr("font-size",          (d) => `${Math.round(radius(d) * 0.65)}px`)
    ...
    node.append("text")
      .attr("text-anchor",       "middle")
      .attr("dominant-baseline", "central")
      .attr("dy",                (d) => `${Math.round(radius(d) * 0.45)}px`)
      .attr("fill",              (d) => d.color)
      .attr("font-size",         "10px")
    ...
    const fitTimer = setTimeout(() => {
      ...
      nodes.forEach((n) => {
        const r = radius(n)
        x0 = Math.min(x0, (n.x ?? 0) - r - pad)
        y0 = Math.min(y0, (n.y ?? 0) - r - pad)
        x1 = Math.max(x1, (n.x ?? 0) + r + pad)
        y1 = Math.max(y1, (n.y ?? 0) + r + pad)
      })
      const bw = x1 - x0, bh = y1 - y0
      const s  = Math.min(w / bw, h / bh, 1.2)
    ...
    onCleanup(() => { sim.stop(); clearTimeout(fitTimer) })
  }

  const loading = () => projects.loading || allSources.loading
  const empty   = () => !loading() && (projects()?.length ?? 0) === 0
      ...
      <div style={{
        ...
        <Show when={!loading()}>
          <span style={{
            ...
            "font-family": "'Geist Mono', monospace", "margin-left": "4px",
          }}>
      ...
      <div style={{ flex: "1", position: "relative", overflow: "hidden" }}>
        ...
        <Show when={empty()}>
          <div style={{ position: "absolute", inset: "0", display: "flex", "flex-direction": "column", "align-items": "center", "justify-content": "center", gap: "10px" }}>
            ...
            <code style={{ "font-size": "12px", color: "#9ca3af", "font-family": "'Geist Mono', monospace", background: "#f3f4f6", padding: "3px 10px", "border-radius": "4px" }}>
              supadense init
            </code>
          </div>
        </Show>
