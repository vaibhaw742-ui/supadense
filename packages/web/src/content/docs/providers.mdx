---
title: Providers
description: Using any LLM provider in Supadense.
---

import config from "../../../config.mjs"
export const console = config.console

Supadense uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Supadense config.
...
You can customize the providers through the `provider` section in your Supadense
config.
...
Supadense Zen is a list of models provided by the Supadense team that have been
tested and verified to work well with Supadense. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Supadense Zen.
:::

1. Run the `/connect` command in the TUI, select `Supadense Zen`, and head to [supadense.com/auth](https://supadense.com/zen).
...
It works like any other provider in Supadense and is completely optional to use.

---

## Supadense Go

Supadense Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Supadense team that have been
tested and verified to work well with Supadense.

1. Run the `/connect` command in the TUI, select `Supadense Go`, and head to [supadense.com/auth](https://supadense.com/zen).
...
To use Amazon Bedrock with Supadense:
...
Supadense. Anthropic explicitly prohibits this.

Previous versions of Supadense came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Supadense with zero setup:
...
GitLab Duo support in Supadense is experimental. Features, configuration, and
behavior may change in future releases.
:::

Supadense integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Supadense uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Supadense
...
When a `duo-workflow-*` model is selected, Supadense will:
...
4. Route tool execution requests through Supadense's permission-gated tool system
...
To use Google Vertex AI with Supadense:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Supadense conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Supadense conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for Supadense. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Supadense:
...
3. Copy the API key for use in Supadense.
...
6. **Important**: Before using cloud models in Supadense, you must pull the model information locally:
...
Supadense Zen is a list of tested and verified models provided by the Supadense team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Supadense Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Supadense Zen**.

   ```txt
   /connect
   ```

3. Enter your Supadense API key.
...
The `limit` fields allow Supadense to understand how much context you have left. Standard providers pull these from models.dev automatically.
