...
There are two types of agents in Supadense; primary agents and subagents.
...
Supadense comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Supadense comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Supadense comes with two built-in primary agents and two built-in subagents.
...
If no temperature is specified, Supadense uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Supadense config and the project specific config.
...
The model ID in your Supadense config uses the format `provider/model-id`. For example, if you're using [Supadense Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
